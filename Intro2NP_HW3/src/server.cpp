...
map<string, int> chatroom_port;
map<string, string> chatroom_status;
...
string server_service(string ip_port, string line) {
  ...
  if (words[0] == "logout") {
    if (login_users.count(ip_port) == 1) {
      if (chatroom_port.count(login_users[ip_port]) == 0 || chatroom_status[login_users[ip_port]] == "close") {
  ...
  if (words[0] == "create-chatroom") {
    if (login_users.count(ip_port) == 1) {
      if (chatroom_port.count(login_users[ip_port]) == 0) {
        // not going to save client's ip because of the spec
        chatroom_port[login_users[ip_port]] = stoi(words[1]);
        chatroom_status[login_users[ip_port]] = "open";
        result << login_users[ip_port] << " " << words[1];
      } else {
  ...
  if (words[0] == "list-chatroom") {
    if (login_users.count(ip_port) == 1) {
      result << left << setw(16) << "Chatroom_name" << "Status" << endl;
      for (auto it = chatroom_status.begin(); it != chatroom_status.end(); it++) {
        result << setw(16) << it->first << it->second << endl;
      }
    } else {
      result << "Please login first." << endl;
    }
  ...
  if (words[0] == "join-chatroom") {
    if (login_users.count(ip_port) == 1) {
      if (chatroom_port.count(words[1]) == 1 && chatroom_status[words[1]] == "open") {
        // not going to send client's ip because of the spec
        result << login_users[ip_port] << " " << chatroom_port[words[1]];
      } else {
  ...
  if (words[0] == "attach") {
    if (login_users.count(ip_port) == 1) {
      if (chatroom_port.count(login_users[ip_port]) == 1) {
        if (chatroom_status[login_users[ip_port]] == "open") {
          // not going to send client's ip because of the spec
          result << login_users[ip_port] << " " << chatroom_port[login_users[ip_port]];
        } else {
  ...
  if (words[0] == "restart-chatroom") {
    if (login_users.count(ip_port) == 1) {
      if (chatroom_port.count(login_users[ip_port]) == 1) {
        if (chatroom_status[login_users[ip_port]] == "close") {
          // not going to send client's ip because of the spec
          chatroom_status[login_users[ip_port]] = "open";
          result << login_users[ip_port] << " " << chatroom_port[login_users[ip_port]];
        } else {
  ...
  if (words[0] == "leave-chatroom") {
    chatroom_status[login_users[ip_port]] = "close";
...
class Server {
 private:
  ...
  static void *tcp_service_thread(void *c_fd_p) {
    ...
    if (chatroom_status.count(tcp_upd[ip_port]) == 1) {
      chatroom_status[tcp_upd[ip_port]] = "close";
    }
  ...
  void handleInput(int c_fd) { //handle input from udp protocol
    sockaddr src_addr;
    socklen_t src_len = sizeof(sockaddr);
      ...
      for (int i = 0; i < pollfds.size(); i++) {
        if ((pollfds[i].revents & POLL_IN) == POLL_IN) {
          if (pollfds[i].fd == s_tcpfd) { // new tcp connection
            handleConnection();
          } else { // recv from client using udp
